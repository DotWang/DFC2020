...
class DF2020(torch.utils.data.Dataset):
    def __init__(self,args,s1,s2,y,split='trn',flag='trg'):
        self.args=args
        self.s1=s1
        self.s2=s2
        self.y=y
    ...
    def filter_label(self,x2,y):
       ...
       # EBSI = (BSI - MNDWI) / (BSI + MNDWI)
        ...
        if self.split=='val' or (self.split=='trn' and self.flag=='trg'):
    ...
    def totensor(self,x1,x2,y):
        ...
        if self.split=='trn' or self.split=='val':
            ...
            y = list(map(lambda x: templab2net[x], y))
