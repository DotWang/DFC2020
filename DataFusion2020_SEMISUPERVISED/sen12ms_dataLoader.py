...
class SEN12MSDataset:
    ...
    def get_s1s2lc_triplet(self, season, scene_id, patch_id, s1_bands=S1Bands.ALL, s2_bands=S2Bands.ALL, lc_bands=LCBands.ALL):
        s1  = self.get_patch(season, scene_id, patch_id, s1_bands)
        s2  = self.get_patch(season, scene_id, patch_id, s2_bands)
        lc  = self.get_patch(season, scene_id, patch_id, lc_bands)
    ...
    def get_triplets(self, season, scene_ids=None, patch_ids=None, s1_bands=S1Bands.ALL, s2_bands=S2Bands.ALL, lc_bands=LCBands.ALL):
        ...
        for sid in scene_list:
            ...
            for pid in patch_list:
                s1, s2, lc = self.get_s1s2lc_triplet(
        ...
        return s1_data, s2_data, lc_data #, bounds
...
if __name__ == "__main__":
    ...
    SCENE_ID = 8
    s2_rgb_patch = sen12ms.get_patch(Seasons.SPRING, SCENE_ID,
                                            spring_ids[SCENE_ID][0], bands=S2Bands.RGB)
    print("Time Taken {}s".format(time.time() - start))
                                            
    print("S2 RGB: {}".format(s2_rgb_patch))
    ...
    for scene_id, patch_ids in spring_ids.items():
        ...
        s1, s2, lc = sen12ms.get_s1s2lc_triplet(Seasons.SPRING, scene_id, patch_ids[0], s1_bands=S1Bands.ALL,
                                                        s2_bands=[S2Bands.red, S2Bands.nir1], lc_bands=LCBands.IGBP)
        print(
            "Scene: {}, S1: {}, S2: {}, LC: {}"
                .format(scene_id, s1, s2, lc))
        i += 1
    ...
    s1, s2, lc  = sen12ms.get_triplets(Seasons.SPRING, s1_bands=S1Bands.ALL,
                                        s2_bands=S2Bands.ALL, lc_bands=LCBands.ALL)

    print("Scene: 106, S1: {}, S2: {}, LC: {}".format(len(s1),len(s2),len(lc)))
    print("Time Taken {}s".format(time.time() - start))
