...
def main():
    ...
    parser.add_argument('--model', type=str, default='deeplabv3',choices=['deeplabv3', 'deeplabfpn','unet'],
    ...
    pre_loader = DataLoader(dataset=pre_dataset, batch_size=args.pre_batch_size)
    ...
    for i, (x1,x2,y,index) in enumerate(tbar):
        ...
        # im[np.where((NBI > 1000) & ((im==1) | (im==4) | (im==5)) | (im==2))] = 9
        # label[np.where((NBI > 1000) & ((label==0) | (label==3) | (label==4) | (label==1)))] = 8

        # yy=y[0,0,:,:].cpu().numpy()
        #
        # im[np.where(GRABS>5.50)]=9
        # im[np.where((yy == 8) & (im!=6) & (im!=7) & (im!=10))]=9
        #
        # label[np.where(GRABS > 5.50)] = 8
        # label[np.where((yy == 8) & (label != 5) & (label != 6) & (label != 9))] = 8


        #森林

        # im[np.where(NDVI > 0.75)] = 1
        # label[np.where(NDVI > 0.75)] = 0

        #草地

        # im[np.where((NDVI>0.4) & (NDVI<0.55) & (np.sum(im==10)<5000))]=4
        # label[np.where((NDVI>0.4) & (NDVI<0.55) & (np.sum(label==9)<5000))]=3

        # im[np.where((NDWI<0) & (NDVI<0.35) & (NDSI>-0.99) & (im==7))]=9
        # label[np.where((NDWI<0) & (NDVI<0.35) & (NDSI>-0.99) & (label==6))]=8


        #im[np.where((NDVI>0.6) & (NDVI<0.75))]=5
        #label[np.where((NDVI>0.6) & (NDVI<0.75))]=4
